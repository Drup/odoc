...
let no_anchor :
   _ -> _ -> string -> (string list, Error.t) result =
  fun k id child ->
  k ~stop_before:false id
  >>= function
  | { page; anchor = ""; _ } -> Ok page
  | otherwise -> Error (Error.Unexpected_anchor (otherwise, child))

let rec from_identifier : stop_before:bool ->
  Identifier.t -> (t, Error.t) result =
  fun ~stop_before ->
  let open Error in
  let k = from_identifier in
  function
  ...
  | `Module (parent, mod_name) ->
    no_anchor k
      (parent :> Identifier.t)
      ("module " ^ ModuleName.to_string mod_name)
    >>| fun parent ->
    let kind = "module" in
    if stop_before then
      { page = parent;
        anchor = Printf.sprintf "%s-%s" kind (ModuleName.to_string mod_name);
        kind }
    else
      { page = (ModuleName.to_string mod_name) :: parent;
        anchor = "";
        kind }
  | `Argument (functor_id, arg_num, arg_name) ->
    no_anchor k
      (functor_id :> Identifier.t)
      ("arg " ^ ArgumentName.to_string arg_name)
    >>| fun parent ->
  ...
  | `ModuleType (parent, modt_name) ->
    no_anchor k
      (parent :> Identifier.t)
      ("module type " ^ ModuleTypeName.to_string modt_name)
    >>| fun parent ->
  ...
  | `Type (parent, type_name) ->
    no_anchor k
      (parent :> Identifier.t)
      ("type " ^ (TypeName.to_string type_name))
    >>| fun page ->
    let kind = "type" in
    { page;
  ...
  | `Constructor (parent, name) ->
    from_identifier ~stop_before:false
      (parent :> Identifier.t)
    >>= begin function
  ...
  | `Class (parent, name) ->
    no_anchor k
      (parent :> Identifier.t)
      ("class " ^ (ClassName.to_string name))
    >>| fun parent ->
    let kind = "class" in
  ...
  | `ClassType (parent, name) ->
    no_anchor k
      (parent :> Identifier.t)
      ("class type " ^ (ClassTypeName.to_string name))
    >>| fun parent ->
...
let rec from_reference : stop_before:bool ->
  Reference.Resolved.t -> (t, Error.t) result =
  fun ~stop_before ->
  let open Error in
  let k = from_reference in
  function
  | `Module (parent, mod_name) ->
    no_anchor k
      (parent :> Reference.Resolved.t)
      ("module " ^ ModuleName.to_string mod_name)
    >>| fun parent ->
    let kind = "module" in
    if stop_before then
  ...
  | `Field (parent, name) ->
    from_reference ~stop_before:false (parent :> Reference.Resolved.t)
    >>= begin function
    (* FIXME: update doc-ock. *)
    (*       | { anchor = ""; _ } as t -> Error (Missing_anchor (t, name)) *)
    | { page; anchor; _ } ->
  ...
  | `Identifier id -> from_identifier ~stop_before:false id
  | `SubstAlias (_, p) -> 
    from_reference ~stop_before:false
      (p :> Reference.Resolved.t)
